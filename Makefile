...
docker-build-api:
	@docker build . -f build/docker/prod/Dockerfile.api -t razorpay/app_name:latest

.PHONY: docker-build-worker
docker-build-worker:
	@docker build . -f build/docker/prod/Dockerfile.webhook.worker -t razorpay/app_name-worker:latest

.PHONY: docker-build-migration
docker-build-migration:
	@docker build . -f build/docker/prod/Dockerfile.migration -t razorpay/app_name-migration:latest

.PHONY: mock-gen ## Generates mocks
mock-gen:
	@mkdir -p pkg/queue/mocks

	@mockgen -destination=pkg/worker/mock/manager.go -package=api github.com/razorpay/trino-gateway/pkg/worker IManager
	@mockgen -destination=pkg/worker/mock/queue/queue.go -package=queue github.com/razorpay/trino-gateway/pkg/worker/queue IQueue
	@mockgen -destination=pkg/worker/mock/logger/logger.go -package=logger github.com/razorpay/trino-gateway/pkg/worker ILogger
