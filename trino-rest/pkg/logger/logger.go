...
func NewLogger(c Config) (*ZapLogger, error) {
	...
	logOnce.Do(func() {
		...
		if config.StackTraceKey == "" {
			config.StackTraceKey = DefaultStacktraceKey
		}

		if config.StackTraceLevel == "" {
			config.StackTraceLevel = DefaultStacktraceLevel
		}

		cores := []zapcore.Core{}
		level := getZapLevel(c.LogLevel)
		cfg := getConfig(level)
		cfg.EncoderConfig.StacktraceKey = config.StackTraceKey
		_, err = cfg.Build()
		if err == nil {
			...
			l := zap.New(combinedCore,
				zap.AddCallerSkip(2),
				zap.AddCaller(),
				zap.AddStacktrace(getZapLevel(config.StackTraceLevel)),
			).Sugar()
